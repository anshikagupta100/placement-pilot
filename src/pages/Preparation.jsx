...
export default function Preparation() {
  ...
  const overall = topics.length ? Math.round(topics.reduce((sum, topic) => sum + topic.progress, 0) / topics.length) : 0;
  const completed = topics.filter((topic) => topic.progress >= 100).length;
  const nextTopic = [...topics].sort((a, b) => a.progress - b.progress)[0];

  const updateProgress = (id) => {
    setTopics((current) => current.map((topic) => topic.id === id ? { ...topic, progress: Math.min(100, topic.progress + 10) } : topic));
  };
      ...
      <main className="dashboard-main">
        ...
        <section className="preparation-hero">
          <div className="preparation-hero-copy">
            ...
            <p>Turn your placement preparation into a clear daily plan. Practice, track progress, and know exactly what to focus on next.</p>
            <div className="preparation-hero-actions">
              <button className="dashboard-primary-button" onClick={() => document.getElementById("preparation-topics")?.scrollIntoView({ behavior: "smooth" })}>Continue learning <span>→</span></button>
              <button className="preparation-outline-button" onClick={() => setShowModal(true)}>+ Add custom topic</button>
            </div>
          </div>
        ...
        <section className="preparation-stats">
          <article className="preparation-stat-card"><div className="preparation-stat-icon blue">◒</div><div><span>Overall progress</span><strong>{overall}%</strong><small>Across all learning areas</small></div></article>
          <article className="preparation-stat-card"><div className="preparation-stat-icon green">✓</div><div><span>Topics completed</span><strong>{completed}<em>/{topics.length}</em></strong><small>Keep making progress</small></div></article>
          <article className="preparation-stat-card"><div className="preparation-stat-icon purple">◷</div><div><span>Study streak</span><strong>7<em> days</em></strong><small>Consistency builds confidence</small></div></article>
          <article className="preparation-stat-card"><div className="preparation-stat-icon orange">★</div><div><span>Next focus</span><strong className="focus-value">{nextTopic?.title || "Start a topic"}</strong><small>{nextTopic ? `${nextTopic.progress}% complete` : "Add your first goal"}</small></div></article>
        </section>
        ...
        <section className="preparation-focus-panel">
          ...
          <div className="focus-progress">
            ...
            <button className="dashboard-secondary-button" disabled={!nextTopic} onClick={() => nextTopic && updateProgress(nextTopic.id)}>{nextTopic?.progress >= 100 ? "Completed ✓" : "Complete 10% practice"}</button>
          </div>
        </section>

        <section className="dashboard-panel preparation-topics-panel" id="preparation-topics">
          <div className="preparation-topics-header">
            <div><span className="preparation-overline">YOUR ROADMAP</span><h2>Preparation areas</h2><p>Work through each area and build confidence step by step.</p></div>
          ...
          <div className="preparation-topic-grid">
            {filteredTopics.map((topic) => (
              <article className="preparation-topic-card" key={topic.id}>
                <div className="topic-card-heading"><div className="preparation-topic-icon">{topic.icon}</div><span className="preparation-category">{topic.category}</span></div>
                <h3>{topic.title}</h3>
                ...
                <div className="topic-card-footer"><span>{topic.progress >= 100 ? "Ready for interview" : `${100 - topic.progress}% remaining`}</span><button onClick={() => updateProgress(topic.id)} disabled={topic.progress >= 100}>{topic.progress >= 100 ? "Completed ✓" : "Practice +10%"}</button></div>
              </article>
            ))}
          </div>

          {filteredTopics.length === 0 && <div className="preparation-empty-state"><div>✦</div><h3>No topics in this category</h3><p>Add a custom preparation topic to expand your roadmap.</p><button className="dashboard-primary-button" onClick={() => setShowModal(true)}>Add a topic</button></div>}
        </section>
      </main>
      ...
      {showModal && (
        <div className="preparation-modal-backdrop" onMouseDown={() => setShowModal(false)}>
          <form className="preparation-modal" onSubmit={addTopic} onMouseDown={(event) => event.stopPropagation()}>
            <div className="modal-heading"><div><span className="preparation-overline">PERSONALIZE</span><h2>Add preparation topic</h2><p>Create a goal that matters for your placement.</p></div><button type="button" className="modal-close" onClick={() => setShowModal(false)}>×</button></div>
